...
function detectRunner(ctx, pkg) {
    const pm = pkg.packageManager ?? "";
    if (pm.startsWith("pnpm") || ctx.exists("pnpm-lock.yaml"))
        return "pnpm";
    if (pm.startsWith("yarn") || ctx.exists("yarn.lock"))
        return "yarn";
    if (pm.startsWith("bun") || ctx.exists("bun.lockb") || ctx.exists("bun.lock"))
        return "bun";
    return "npm";
}
/** Distinguish vitest vs jest from the test script and deps, so scoping/parsing can target it. */
function testFramework(pkg, testScript) {
    const deps = { ...pkg.dependencies, ...pkg.devDependencies };
    const s = testScript ?? "";
    if (/\bvitest\b/.test(s) || "vitest" in deps)
        return "vitest";
    if (/\bjest\b/.test(s) || "jest" in deps)
        return "jest";
    return undefined;
}
