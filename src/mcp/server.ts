...
const KINDS = ["typecheck", "lint", "test", "build", "custom"] as const;
...
export function createServer(): McpServer {
  ...
  server.registerTool(
    "run_checks",
    {
      title: "Run checks",
      description:
        "Run the project's checks and return pass/fail plus the output of any failing check. Use after changing code; if it fails, fix the root cause and run again.",
      inputSchema: {
    ...
    async ({ cwd, kinds, stopOnFirstFailure, affectedFiles, since }) => {
      ...
      if (kinds && kinds.length > 0) checks = checks.filter((c) => kinds.includes(c.kind));
